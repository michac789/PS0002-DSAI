"
K-Nearest-Neighbors (KNN) Regresion 1
"
...
set.seed(100)
training_idx <- sample(seq(1, nrow(Boston)), nrow(Boston) * 0.8)
train_data <- Boston[training_idx, ]
test_data <- Boston[-training_idx, ]

# 4. Fit the model
set.seed(101)
model <- train(
    medv~., data = train_data, method = "knn",
...
plot(test_data$medv, predictions,
    main = "Prediction performance of kNN regression")
abline(0, 1, col = "red")
